...
static inline emitted_hit _emitted_hit(vec3 pos, vec3 col) {
  emitted_hit ret = {
    .pos = pos, .col = col
  };
  return ret;
...
static void emitlightrays(const XXscene * scene, emitted_hit * out_emitnodes, int * nodecount);
static void emitray(vec3 r_p, vec3 r_d, XXsceneobj * emitter, emitted_hit * out_emitnodes, int * nodecount);

XXscene g_scene;
emitted_hit * emitnodesarr;
...
static void init() {
  ...
  emitnodesarr = xxalloc(MAX_EMITTED_HITS * sizeof(emitted_hit));
...
static void renderScene(const XXscene * scene, char * out_data, int w, int h, xxfloat t) {
  ...
  int nodecount = 0;
  emitlightrays(scene, emitnodesarr, &nodecount);
  ...
  for(int y = 0; y < h; y++) {
    for(int x = 0; x < w; x++) { 
        ...
        *(buf++) = evalpixel(cam_p, r_d, cam_d, &g_scene);
    }
  }
}
...
static void emitlightrays(const XXscene * scene, emitted_hit * out_emitnodes, int * nodecount) {
  ...
  int obj_i = 0;
  double ity_i = 0.0f;
  double ity_d = (double)prev_ity / EMIT_INITIAL_RAYS;
  for(int i = 0; i < EMIT_INITIAL_RAYS; i++) {
    ity_i += ity_d;
    while(emitter_intensities[obj_i] < ity_i) {
      obj_i++;
    }
    XXsceneobj * obj = &scene->objs[i];
    ...
    vec3_normalize(&r_d);
    r_d = genhemisray(r_d);
    emitray(r_p, r_d, obj, out_emitnodes, nodecount);
  }
}
  ...
  if (isect_scene(scene, r_p, r_d, &hitobj, &hitpos, &hitnrm)) {
    ...
    if (depth == 0) {
      for (int _i = 0; _i < SAMPLES_PER_PIXEL; _i++) {
        ...
        rad = vec3_add(rad, computeradiance(hitpos, nr, scene, depth+1));
      }
      rad = vec3_mul(rad, 1.0f / SAMPLES_PER_PIXEL);
    } else
    */
    {
      vec3 nr = genhemisray(hitnrm);
...
static inline vec3 pickspherepos() {
  ...
  xxfloat a = 2*M_PI*randf();
  xxfloat r2 = sqrtf(y*y - 1);
...
static inline unsigned int evalpixel(vec3 r_p, vec3 r_d, vec3 cam_d, const XXobj * obj) { 
  ...
  if (isect_obj(obj, r_p, r_d, &pos, &nrm)) {
    xxfloat f = (-vec3_dot(nrm, cam_d)) + 0.2f;
...
static void allocrandomscene(int obj_count, XXscene * out_scene) {
  *out_scene = scene_alloc(obj_count+1);
  ...
  for (int i = 0; i < obj_count-1; i++) {
    ...
    vec3 emit = vec3_mul(obj_col, i <= 4 ? 0.75f : 0.01f);
    XXsceneobj obj = {
